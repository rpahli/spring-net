...
namespace Spring.Util.Generic
{
    ...
    public sealed class CollectionUtils
    {
        ...
        public static bool Contains<T>(ICollection<T> collection, object element)
        {
            ...
            MethodInfo method;
            method = collection.GetType().GetMethod("contains",
                BindingFlags.IgnoreCase | BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public);
            if (null == method)
            {
                throw new InvalidOperationException("Collection type " + collection.GetType() +
                                                    " does not implement a Contains() method.");
            }
