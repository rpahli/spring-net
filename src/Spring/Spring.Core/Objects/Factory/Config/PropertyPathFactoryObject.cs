...
namespace Spring.Objects.Factory.Config
{
    ...
    public class PropertyPathFactoryObject : IFactoryObject, IObjectNameAware, IObjectFactoryAware
    {
        ...
        {
            set { targetObjectWrapper = new ObjectWrapper(value); }
        }
        ...
        {
            IObjectWrapper target = targetObjectWrapper;
            if (target == null)
            {
                // fetch the prototype object object...
                target = new ObjectWrapper(objectFactory[targetObjectName]);
            }
            object value = target.GetPropertyValue(propertyPath);
            if (value == null)
            {
                throw new FatalObjectException("PropertyPathFactoryObject is not allowed to return null, " +
                                               "but property value for path '" + propertyPath + "' is null.");
            }
            return value;
        }
        ...
        public IObjectFactory ObjectFactory
        {
            ...
            {
                objectFactory = value;
                if (targetObjectWrapper != null && targetObjectName != null)
                {
                    throw new ArgumentException(
                        "Only one of the TargetObjectName or TargetObject properties can be set, not both.");
                }
                if (targetObjectWrapper == null && targetObjectName == null)
                {
                    ...
                    string strippedObjectname = objectName.Trim();
                    int dotIndex = strippedObjectname.IndexOf('.');
                    if (dotIndex <= 0)
                    {
                        throw new ArgumentException(
                            "Neither TargetObject nor TargetObjectName specified, and PropertyPathFactoryObject " +
                            "object name '" + objectName + "' does not follow 'objectName.property' syntax.");
                    }
                    targetObjectName = strippedObjectname.Substring(0, dotIndex);
                    propertyPath = strippedObjectname.Substring(dotIndex + 1);
                }
                ...
                if (targetObjectWrapper == null
                    && objectFactory.IsSingleton(targetObjectName))
                {
                    // eagerly fetch singleton target object, and determine result type...
                    targetObjectWrapper
                        = new ObjectWrapper(objectFactory.GetObject(targetObjectName));
                    ObjectType = targetObjectWrapper.GetPropertyType(propertyPath);
                }
            }
        }
