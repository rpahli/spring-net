...
namespace Spring.Objects.Factory.Support
{
    ...
    public class ConstructorResolver
    {
            ...
            ConstructorInfo[] chosenCtors, object[] explicitArgs)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug(string.Format(CultureInfo.InvariantCulture,
                    "Object '{0}' instantiated via constructor [{1}].", objectName,
                    constructorInstantiationInfo.ConstructorInfo));
            }
        ...
        /// </returns>
        public virtual IObjectWrapper InstantiateUsingFactoryMethod(string name, RootObjectDefinition definition,
            object[] arguments)
        {
            ObjectWrapper wrapper = new ObjectWrapper();
            ...
            IList<MethodInfo> factoryMethodCandidates = FindMethods(genericArgsInfo.GenericMethodName, expectedArgCount,
                isStatic, factoryClass);
            ...
            for (int i = 0; i < factoryMethodCandidates.Count; i++)
            {
                ...
                object objectInstance = instantiationStrategy.Instantiate(definition, name, objectFactory,
                    factoryMethodCandidate, arguments);
                ...
                {
                    log.Debug(string.Format(CultureInfo.InvariantCulture,
                        "Object '{0}' instantiated via factory method [{1}].", name, factoryMethodCandidate));
                }
            ...
            throw new ObjectDefinitionStoreException(
                string.Format(CultureInfo.InvariantCulture, "Cannot find matching factory method '{0} on Type [{1}].",
        ...
        /// </remarks>
        private ArgumentsHolder CreateArgumentArray(string objectName, RootObjectDefinition rod,
            ConstructorArgumentValues resolvedValues, ObjectWrapper wrapper, Type[] paramTypes,
            MethodBase methodOrCtorInfo, bool autowiring,
            out UnsatisfiedDependencyExceptionData unsatisfiedDependencyExceptionData)
        {
            ...
            for (int paramIndex = 0; paramIndex < paramTypes.Length; paramIndex++)
            {
                ...
                if (valueHolder != null)
                {
                    ...
                    try
                    {
                        ...
                        object convertedValue =
                            TypeConversionUtils.ConvertValueIfNecessary(paramType, originalValue, null);
                    ...
                    catch (TypeMismatchException ex)
                    {
                        ...
                        unsatisfiedDependencyExceptionData =
                            new UnsatisfiedDependencyExceptionData(paramIndex, paramType, errorMessage);
                        return null;
                    }
                }
                else
                {
                    ...
                    {
                        unsatisfiedDependencyExceptionData =
                            new UnsatisfiedDependencyExceptionData(paramIndex, paramType, ex.Message);
        ...
        private object ResolveAutoWiredArgument(MethodParameter methodParameter, string objectName,
            IList autowiredObjectNames)
        {
            return
                autowireFactory.ResolveDependency(new DependencyDescriptor(methodParameter, true), objectName,
...
//            ObjectDefinitionValueResolver valueResolver = new ObjectDefinitionValueResolver(objectFactory);
            ...
            foreach (ConstructorArgumentValues.ValueHolder valueHolder in definition.ConstructorArgumentValues
            ...
            foreach (KeyValuePair<string, object> namedArgumentEntry in definition.ConstructorArgumentValues
                .NamedArgumentValues)
            {
                ...
                object resolvedValue =
                    valueResolver.ResolveValueIfNecessary(objectName, definition, syntheticArgumentName,
                        valueHolder.Value);
        ...
        ///     <paramref name="searchType" /> that match the supplied criteria.
        /// </returns>
        private static IList<MethodInfo> FindMethods(string methodName, int expectedArgumentCount, bool isStatic,
            Type searchType)
        {
            ...
            BindingFlags methodFlags = BindingFlags.Public | BindingFlags.IgnoreCase |
                                       (isStatic ? BindingFlags.Static : BindingFlags.Instance);
            MemberInfo[] methods = searchType.FindMembers(MemberTypes.Method, methodFlags,
                new CriteriaMemberFilter().FilterMemberByCriteria, methodCriteria);
