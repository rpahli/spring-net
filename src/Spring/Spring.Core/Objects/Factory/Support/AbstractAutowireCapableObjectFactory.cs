...
namespace Spring.Objects.Factory.Support
{
    ...
    public abstract class AbstractAutowireCapableObjectFactory : AbstractObjectFactory, IAutowireCapableObjectFactory
    {
        ...
        protected const BindingFlags MethodResolutionFlags =
            BindingFlags.Public | BindingFlags.InvokeMethod | BindingFlags.Static | BindingFlags.Instance |
        ...
        {
            ApplyPropertyValues(name, new RootObjectDefinition(definition), new ObjectWrapper(instance),
        ...
        protected object ApplyObjectPostProcessorsBeforeInstantiation(Type objectType, string objectName)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug(string.Format(
                    "Invoking IInstantiationAwareObjectPostProcessors before " + "the instantiation of '{0}'.",
                    objectName));
            }
        ...
        {
            log.Debug($"Invoking PostProcessBeforeDestruction after IDisposal of object '{name}'");
            ...
            foreach (IObjectPostProcessor objectProcessor in ObjectPostProcessors)
                if (objectProcessor is IDestructionAwareObjectPostProcessor)
                {
                    try
                    {
                        ((IDestructionAwareObjectPostProcessor) objectProcessor).PostProcessBeforeDestruction(instance,
                            name);
                    }
                    catch (Exception ex)
                    {
        ...
        ///     </p>
        /// </remarks>
        protected void ApplyPropertyValues(string name, RootObjectDefinition definition, IObjectWrapper wrapper,
            IPropertyValues properties)
        {
            ...
            for (int i = 0; i < copiedProperties.Count; ++i)
            {
                ...
                object value =
                    valueResolver.ResolveValueIfNecessary(name, definition, copiedProperty.Name, copiedProperty.Value);
        ...
        public override void Dispose()
        {
            ...
            foreach (object o in clone)
                DestroyObject(
                    string.Format(CultureInfo.InvariantCulture, "(Inner object of Type '{0}')", o.GetType().FullName),
                    o);
        ...
        protected void PopulateObject(string name, RootObjectDefinition definition, IObjectWrapper wrapper)
        {
            ...
            if (definition.ResolvedAutowireMode == AutoWiringMode.ByName ||
                definition.ResolvedAutowireMode == AutoWiringMode.ByType)
            {
            ...
            if (hasInstAwareOpps || needsDepCheck)
            {
                ...
                if (hasInstAwareOpps)
                {
                    foreach (IObjectPostProcessor processor in ObjectPostProcessors)
                    {
                        ...
                        {
                            properties =
                                instantiationAwareObjectPostProcessor.PostProcessPropertyValues(properties,
                                    filteredPropInfo, wrapper.WrappedInstance,
                                    name);
                            if (properties == null)
                            {
                                return;
        ...
        protected void WireEvents(string name, IConfigurableObjectDefinition definition, IObjectWrapper wrapper)
        {
                ...
                in definition.EventHandlerValues[eventName])
            {
                ...
                {
                    throw new FatalObjectException("Currently, only references to other objects and Types are " +
                                                   "supported as event sources.");
                }
        ...
        /// </param>
        protected void AutowireByName(string name, RootObjectDefinition definition, IObjectWrapper wrapper,
            ...
            foreach (string propertyName in propertyNames)
                    ...
                    if (log.IsDebugEnabled)
                    {
                        log.Debug(
                            string.Format(CultureInfo.InvariantCulture,
                                "Added autowiring by name from object name '{0}' via " +
                                "property '{1}' to object named '{1}'.", name,
                                propertyName));
                    }
                ...
                else
                {
                    ...
                    if (log.IsDebugEnabled)
                    {
                        log.Debug(
                            string.Format(CultureInfo.InvariantCulture,
                                "Not autowiring property '{0}' of object '{1}' by name: " + "no matching object found.",
                                propertyName, name));
                    }
        ...
        ///     The property values to register wired objects with.
        /// </param>
        protected void AutowireByType(string name, RootObjectDefinition definition, IObjectWrapper wrapper,
            MutablePropertyValues properties)
        {
            string[] propertyNames = UnsatisfiedNonSimpleProperties(definition, wrapper);
            foreach (string propertyName in propertyNames)
            {
                ...
                if (matchingObjects != null && matchingObjects.Count == 1)
                {
                    ...
                    if (log.IsDebugEnabled)
                    {
                        log.Debug(
                            string.Format(CultureInfo.InvariantCulture,
                                "Autowiring by type from object name '{0}' via property " +
                                "'{1}' to object named '{2}'.", name,
                ...
                else
                {
                    ...
                    if (log.IsDebugEnabled)
                    {
                        log.Debug(
                            string.Format(CultureInfo.InvariantCulture,
                                "Not autowiring property '{0}' of object '{1}': no matching object found.",
                                propertyName, name));
                    }
        ...
        protected internal override object InstantiateObject(string name, RootObjectDefinition definition,
            object[] arguments, bool allowEagerCaching, bool suppressConfigure)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug(string.Format(CultureInfo.InvariantCulture,
                    "Creating instance of Object '{0}' with merged definition [{1}].", name, definition));
            }
            ...
            try
            {
                ...
                if (allowEagerCaching && definition.IsSingleton)
                {
                    if (log.IsDebugEnabled)
                    {
                        log.Debug("Eagerly caching object '" + name +
            ...
            catch (Exception ex)
            {
                ...
                throw new ObjectCreationException(definition.ResourceDescription, name,
                    "Initialization of object failed : " + ex.Message, ex);
            }
            return instance;
        }
        ...
        /// </remarks>
        protected virtual void AddEagerlyCachedSingleton(string objectName, IObjectDefinition objectDefinition,
            object rawSingletonInstance)
        {
            AddSingleton(objectName, rawSingletonInstance);
        }
        ...
        /// <summary>
        ///     Creates an <see cref="IObjectWrapper" /> instance from the <see cref="RootObjectDefinition" /> passed in
        ///     <paramref name="objectDefinition" />
        ...
        protected IObjectWrapper CreateObjectInstance(string objectName, RootObjectDefinition objectDefinition,
            object[] arguments)
        {
        ...
        protected virtual ConstructorInfo[] DetermineConstructorsFromObjectPostProcessors(Type objectType,
            string objectName)
        {
            if (HasInstantiationAwareBeanPostProcessors)
            {
        ...
        /// </returns>
        protected virtual IObjectWrapper InstantiateUsingFactoryMethod(string name, RootObjectDefinition definition,
            object[] arguments)
        {
        ...
        /// <param name="explicitArgs">
        ///     The argument values passed in programmatically via the GetObject method,
        ///     or <code>null</code> if none (-> use constructor argument values from object definition)
        /// </param>
        /// <returns>
        ...
        protected IObjectWrapper AutowireConstructor(string name, RootObjectDefinition definition,
            ConstructorInfo[] ctors, object[] explicitArgs)
        {
            ConstructorResolver constructorResolver =
                new ConstructorResolver(this, this, InstantiationStrategy, CreateValueResolver());
        ...
        /// </exception>
        protected void DependencyCheck(string name, IConfigurableObjectDefinition definition, IObjectWrapper wrapper,
            IPropertyValues properties)
        {
            ...
            if (HasInstantiationAwareBeanPostProcessors)
            {
                foreach (IObjectPostProcessor processor in ObjectPostProcessors)
                {
                    ...
                    {
                        properties =
                            inProc.PostProcessPropertyValues(properties, filteredPropInfo, wrapper.WrappedInstance,
                                name);
                        if (properties == null)
                        {
                            return;
        ...
        private void CheckDependencies(string name, IConfigurableObjectDefinition definition,
            IList<PropertyInfo> filteredPropInfo, IPropertyValues properties)
        {
            DependencyCheckingMode dependencyCheck = definition.DependencyCheck;
            IList<PropertyInfo> unsatisfiedDependencies =
                AutowireUtils.GetUnsatisfiedDependencies(filteredPropInfo, properties, dependencyCheck);

            if (unsatisfiedDependencies.Count > 0)
            {
                throw new UnsatisfiedDependencyException(definition.ResourceDescription, name,
                    unsatisfiedDependencies[0].Name,
        ...
        protected virtual void InvokeInitMethods(object target, string name, IConfigurableObjectDefinition definition)
        {
            if (ObjectUtils.IsAssignableAndNotTransparentProxy(typeof(IInitializingObject), target))
            {
                ...
                {
                    log.Debug(string.Format(CultureInfo.InvariantCulture,
                        "Calling AfterPropertiesSet() on object with name '{0}'.", name));
                }
            ...
            if (StringUtils.HasText(definition.InitMethodName))
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug(
                        string.Format(CultureInfo.InvariantCulture,
                            "Calling custom init method '{0} on object with name '{1}'.",
                ...
                try
                {
                    MethodInfo targetMethod = target.GetType().GetMethod(definition.InitMethodName,
                        MethodResolutionFlags, null, Type.EmptyTypes, null);
                ...
                {
                    throw new ObjectCreationException(definition.ResourceDescription, name,
                        "Initialization method '" + definition.InitMethodName + "' threw exception",
                        ex.GetBaseException());
                }
                catch (Exception ex)
                {
        ...
        protected virtual void InvokeCustomDestroyMethod(string name, object target, string destroyMethodName)
        {
            ...
            MethodInfo targetMethod = target.GetType()
                .GetMethod(destroyMethodName, MethodResolutionFlags, null, Type.EmptyTypes, null);
            if (targetMethod == null)
            {
                // #%&^! try to find the method with a boolean "force" parameter
                targetMethod = target.GetType().GetMethod(destroyMethodName, MethodResolutionFlags, null,
            ...
            else
            {
                ...
                catch (TargetInvocationException ex)
                {
                    ...
                    log.Error(
                        "Couldn't invoke destroy method '" + destroyMethodName + "' of object with name '" + name + "'",
                        ex.GetBaseException());
        ...
        private void LogExceptionRaisedByCustomDestroyMethodInvocation(string destroyMethodName, string name,
            Exception ex)
        {
            log.Error(
                string.Format(CultureInfo.InvariantCulture,
                    "Couldn't invoke destroy method '{0}' of object with name '{1}'.", destroyMethodName, name),
                ex);
        }
        ...
        {
            using (new DisposableObjectAdapter(target, name, GetMergedObjectDefinition(name, true),
        ...
        protected object ResolveReference(IConfigurableObjectDefinition definition, string name, string argumentName,
            RuntimeObjectReference reference)
        {
            #region Instrumentation
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug(
                    string.Format(CultureInfo.InvariantCulture,
                        "Resolving reference from property '{0}' in object '{1}' to object '{2}'.",
            ...
            {
                throw ObjectCreationException.GetObjectCreationException(ex, name, argumentName,
                    definition.ResourceDescription, reference.ObjectName);
            }
        }
        ...
        protected virtual object ConfigureObject(string name, RootObjectDefinition definition, IObjectWrapper wrapper)
        {
            ...
            if (ObjectUtils.IsAssignableAndNotTransparentProxy(typeof(IObjectNameAware), instance))
            {
                ...
                {
                    log.Debug(string.Format(CultureInfo.InvariantCulture,
                        "Setting the name property on the IObjectNameAware object '{0}'.", name));
                }
            ...
            if (ObjectUtils.IsAssignableAndNotTransparentProxy(typeof(IObjectFactoryAware), instance))
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug(
                        string.Format(CultureInfo.InvariantCulture,
                            "Setting the ObjectFactory property on the IObjectFactoryAware object '{0}'.",
                            name));
                }
        ...
        /// </summary>
        private readonly IDictionary<Type, IList<PropertyInfo>> filteredPropertyDescriptorsCache =
        ...
        /// </summary>
        private readonly ISet ignoredDependencyInterfaces = new HybridSet();
