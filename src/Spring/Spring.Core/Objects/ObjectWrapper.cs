...
namespace Spring.Objects
{
    ...
    {
        private readonly ILogger Log = LogManager.GetLogger(typeof(ObjectWrapper));
        ...
        public virtual void SetPropertyValues(IPropertyValues propertyValues, bool ignoreUnknown)
        {
            ...
            foreach (PropertyValue pv in propertyValues)
                ...
                {
                    Log.Error(
                        string.Format("Failed setting property '{0}' on instance of type '{1}'", pv.Name,
                            WrappedType.FullName), ex);
                    throw;
                }
        ...
        {
            MemberInfo memberInfo = GetPropertyOrFieldInfo(propertyName);
        ...
        /// <exception cref="FatalObjectException">
        ///     If <paramref name="propertyOrFieldName" /> does not resolve to a property or
        ///     field.
        /// </exception>
        private MemberInfo GetPropertyOrFieldInfo(string propertyOrFieldName)
        {
            if (StringUtils.IsNullOrEmpty(propertyOrFieldName))
            {
                throw new FatalObjectException(
                    "Can't find property or field info for null or zero length property name.");
            }
            ...
            try
            {
                ...
                {
                    return ((PropertyOrFieldNode) propertyExpression).GetMemberInfo(wrappedObject);
                }
                if (propertyExpression is IndexerNode)
                {
                    return ((IndexerNode) propertyExpression).GetPropertyInfo(wrappedObject, null);
                }
                if (propertyExpression is Expression)
                {
                    return ((Expression) propertyExpression).GetPropertyInfo(wrappedObject, null);
                }
                throw new FatalObjectException("'" + propertyOrFieldName +
                                               "' is not a valid property or field expression.");
            }
            catch (RecognitionException e)
            {
                throw new FatalObjectException("Failed to parse property or field name '" + propertyOrFieldName + "'.",
                    e);
            }
            catch (TokenStreamRecognitionException e)
            {
