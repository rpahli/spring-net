...
namespace Spring.Context.Support
{
        ...
        : ConfigurableResourceLoader, IConfigurableApplicationContext, IObjectDefinitionRegistry
    {
        ...
        ///     that will be applied to the objects created with this factory.
        /// </value>
        private IList<IObjectFactoryPostProcessor> ObjectFactoryPostProcessors { get; }
        ...
        /// <summary>
        ///     Protects access to the internal object factory used by the ApplicationContext if attempted to be accessed when in
        ///     improper state.
        /// </summary>
        ...
        protected virtual void OnContextEvent(object source, ApplicationEventArgs e)
        {
            ...
            if (exceptions.HasExceptions)
            {
                ...
                throw new ApplicationContextException(
                    string.Format(
                        "An unhandled exception occured during processing application event {0} in handler {1}",
                        e.GetType(), target.Method), exception);
            }
        }
        ...
        private void InvokeObjectFactoryPostProcessors(IConfigurableListableObjectFactory objectFactory)
        {
            ...
            if (objectFactory is IObjectDefinitionRegistry)
            {
                ...
                List<IObjectDefinitionRegistryPostProcessor> registryPostProcessors =
                    new List<IObjectDefinitionRegistryPostProcessor>();

                foreach (IObjectFactoryPostProcessor factoryProcessor in ObjectFactoryPostProcessors)
                {
                    IObjectDefinitionRegistryPostProcessor registryPostProcessor =
                        factoryProcessor as IObjectDefinitionRegistryPostProcessor;
                ...
                IDictionary<string, IObjectDefinitionRegistryPostProcessor> objectMap =
                    objectFactory.GetObjects<IObjectDefinitionRegistryPostProcessor>(true, false);

                List<IObjectDefinitionRegistryPostProcessor> registryPostProcessorObjects =
                    new List<IObjectDefinitionRegistryPostProcessor>(objectMap.Values);
            ...
            List<IObjectFactoryPostProcessor>
                priorityOrderedFactoryProcessors = new List<IObjectFactoryPostProcessor>();
            ...
            for (int i = 0; i < factoryProcessorNames.Count; ++i)
            {
                ...
                {
                    priorityOrderedFactoryProcessors.Add(SafeGetObjectFactory()
                        .GetObject<IObjectFactoryPostProcessor>(processorName));
                }
            ...
            foreach (string orderedFactoryProcessorsName in orderedFactoryProcessorsNames)
                orderedFactoryProcessors.Add(SafeGetObjectFactory()
                    .GetObject<IObjectFactoryPostProcessor>(orderedFactoryProcessorsName));
            ...
            foreach (string nonOrderedFactoryProcessorName in nonOrderedFactoryProcessorNames)
                nonOrderedPostProcessors.Add(SafeGetObjectFactory()
                    .GetObject<IObjectFactoryPostProcessor>(nonOrderedFactoryProcessorName));
        ...
        protected virtual void InvokePriorityOrderedObjectFactoryPostProcessors(List<string> factoryProcessorNames,
            List<IObjectFactoryPostProcessor> priorityOrderedFactoryProcessors)
        {
            ...
            IList<string> factoryProcessorNamesAfterTypeAlias =
            ...
            foreach (string factoryProcessorName in factoryProcessorNamesAfterTypeAlias)
                if (!factoryProcessorNames.Contains(factoryProcessorName))
                {
                    ...
                    {
                        priorityOrderedFactoryProcessors.Add(SafeGetObjectFactory()
                            .GetObject<IObjectFactoryPostProcessor>(factoryProcessorName));
                    }
                }
        ...
        {
            int registeredObjectPostProcessorCount =
                GetObjectNamesForType(typeof(IObjectPostProcessor), true, false).Count;
            int objectPostProcessorCount = SafeGetObjectFactory().ObjectPostProcessorCount + 1
                                           + registeredObjectPostProcessorCount;
            ((ObjectPostProcessorChecker) _defaultObjectPostProcessors[0]).Reset(objectFactory,
                objectPostProcessorCount);
        }
        ...
        {
            ICollection<IApplicationEventListener>
                listeners = GetObjects<IApplicationEventListener>(true, false).Values;
            ...
            IApplicationContext parentApplicationContext)
        {
            ...
            Name = StringUtils.IsNullOrEmpty(name) ? DefaultRootContextName : name;
            IsCaseSensitive = caseSensitive;
            _parentApplicationContext = parentApplicationContext;
            EventRaiser = CreateEventRaiser();
            ObjectFactoryPostProcessors = new List<IObjectFactoryPostProcessor>();
            ...
            AddDefaultObjectPostProcessor(
                new SharedStateAwareProcessor(new ISharedStateFactory[] {new ByTypeSharedStateFactory()},
                    int.MaxValue));
        }
        ...
        ///     all default <see cref="IObjectPostProcessor" />s are registered with the underlying
        ///     <see cref="IConfigurableObjectFactory" />.
        /// </remarks>
        ...
        public void Refresh()
        {
            lock (SyncRoot)
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug("ApplicationContext Refresh: Refreshing object factory ");
                }
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug("ApplicationContext Refresh: Registering well-known processors and objects");
                }
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug("ApplicationContext Refresh: Custom post processing object factory");
                }
                ...
                {
                    log.Debug(
                        "ApplicationContext Refresh: Post processing object factory using pre-registered processors");
                }
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug("ApplicationContext Refresh: Post processing object factory using defined processors");
                }
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug("ApplicationContext Refresh: Preinstantiating singletons");
                }
                ...
                if (log.IsInfoEnabled)
                {
                    log.Info("ApplicationContext Refresh: Completed");
                }
        ...
        ///     <see cref="IConfigurableListableObjectFactory.RegisterResolvableDependency" />
        /// </summary>
        ...
        ///         Use
        ///         <see
        ///             cref="ObjectFactoryUtils.ObjectNamesForTypeIncludingAncestors(Spring.Objects.Factory.IListableObjectFactory,System.Type,bool,bool)" />
        ...
        /// <returns>
        ///     The names of all objects defined in this factory, if <code>includeAncestors</code> is <code>true</code> includes
        ///     all
        ...
        public T GetObject<T>()
        {
            ...
            {
                throw new NoSuchObjectDefinitionException(typeof(T).FullName,
                    "Requested Type not Defined in the Context.");
            }

            if (objectNamesForType.Count > 1)
            {
                throw new ObjectDefinitionStoreException(
                    string.Format("More than one definition for {0} found in the Context.", typeof(T).FullName));
            }
        ...
        {
            get { return SafeGetObjectFactory().GetObject(name); }
        }
        ...
        /// </returns>
        /// <exception cref="NoSuchObjectDefinitionException">
