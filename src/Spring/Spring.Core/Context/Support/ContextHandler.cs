...
namespace Spring.Context.Support
{
    ...
    public class ContextHandler : IConfigurationSectionHandler
    {
        ...
        public object Create(object parent, object configContext, XmlNode section)
        {
            ...
            {
                throw ConfigurationUtils.CreateConfigurationException(
                    string.Format("Parent context must be of type IApplicationContext, but was '{0}'",
        ...
        protected virtual void CreateChildContexts(IApplicationContext parentContext, object configContext,
            IList<XmlNode> childContexts)
        {
            // create child contexts for 'the most recently created context'...
            foreach (XmlNode childContext in childContexts)
                Create(parentContext, configContext, childContext);
        }

        /// <summary>
        ///     Instantiates a new context.
        /// </summary>
        protected virtual IApplicationContext InstantiateContext(IApplicationContext parentContext,
            object configContext, string contextName, Type contextType, bool caseSensitive, IList<string> resources)
        {
            ...
            {
                instantiator = new RootContextInstantiator(contextType, contextName, caseSensitive,
                    new List<string>(resources).ToArray());
            }
            else
            {
                instantiator = new DescendantContextInstantiator(parentContext, contextType, contextName, caseSensitive,
                    new List<string>(resources).ToArray());
            }
        ...
        private Type GetContextType(XmlElement contextElement, IApplicationContext parentContext)
        {
            ...
            {
                contextType = GetConfiguredContextType(contextElement, DefaultApplicationContextType);
            }
            return contextType;
        }
        ...
        private bool GetCaseSensitivity(XmlElement contextElement)
        {
            ...
            {
                caseSensitive = bool.Parse(caseSensitiveAttr);
            }
        ...
        private abstract class ContextInstantiator
        {
            ...
            protected IList<string> Resources { get; }
            ...
            public IApplicationContext InstantiateContext()
            {
                ...
                if (ctor == null)
                {
                    string errorMessage = "No constructor with string[] argument found for context type [" +
                                          ContextType.Name + "]";
        ...
        private sealed class RootContextInstantiator : ContextInstantiator
        {
            ...
            {
                return ContextType.GetConstructor(new[] {typeof(string), typeof(bool), typeof(string[])});
            }

            protected override IApplicationContext InvokeContextConstructor(
                ConstructorInfo ctor)
            {
                return (IApplicationContext) new SafeConstructor(ctor).Invoke(new object[]
                    {ContextName, CaseSensitive, Resources});
            }
        }
        ...
        private sealed class DescendantContextInstantiator : ContextInstantiator
        {
            ...
            protected override ConstructorInfo GetContextConstructor()
            {
                return ContextType.GetConstructor(
                    new[] {typeof(string), typeof(bool), typeof(IApplicationContext), typeof(string[])});
            }
            ...
            {
                return (IApplicationContext) new SafeConstructor(ctor).Invoke(new object[]
                    {ContextName, CaseSensitive, parentContext, Resources});
            }
        }
