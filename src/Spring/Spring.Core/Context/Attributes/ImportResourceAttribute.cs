...
{
    /// <summary>
    ///     Supports providing one or more <see cref="IResource" /> implementations to import when creating
    ///     <see cref="RootObjectDefinition" />s.
    /// </summary>
            ...
            set
            {
                if (!typeof(IObjectDefinitionReader).IsAssignableFrom(value))
                {
                    throw new ArgumentException(string.Format(
                        "DefinitionReader must be of type IObjectDefinitionReader but was of type {0}", value.Name));
                }
