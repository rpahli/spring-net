    ...
    public abstract class AbstractSimpleBinding : AbstractBinding
    {
            ...
            IDictionary<string, object> variables)
        {
            if (IsValid(validationErrors)
                && (Direction == BindingDirection.Bidirectional || Direction == BindingDirection.SourceToTarget))
            {
                try
                {
        ...
        {
            object value = GetSourceValue(source, variables);
            if (Formatter != null && value is string)
            {
                value = Formatter.Parse((string) value);
            }
            SetTargetValue(target, value, variables);
        }
        ...
        {
            if (IsValid(validationErrors)
                && (Direction == BindingDirection.Bidirectional || Direction == BindingDirection.TargetToSource))
            {
                try
                {
                ...
                {
                    log.Warn(string.Format("Failed binding[{0}]:{1}", Id, ex));
                    if (!SetInvalid(validationErrors)) throw;
                }
            }
        }
        ...
        {
            object value = GetTargetValue(target, variables);
            if (Formatter != null)
            {
                value = Formatter.Format(value);
            }
            SetSourceValue(source, value, variables);
        }
